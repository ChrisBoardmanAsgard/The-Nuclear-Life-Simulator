...
public class Artificial_life_simulator extends PApplet {
...
class ControlPanel {
    ...
    public void addButton(String label, int x, int y, int width, int height, CallbackListener callback) {
...
class Environment {
    ...
    public void updateCycle() {
        ...
        if (rainTimer <= 0 && random(1) < 0.001f * simulationSpeed) {
            isRaining = true;
...
class Gene {
    ...
    public Gene mutate() {
        return new Gene(
            constrain(size + random(-0.05f, 0.05f), 1, 5),
            constrain(speed + random(-0.05f, 0.05f), 0.5f, 3),
            ...
            constrain(metabolism + random(-0.02f, 0.02f), 0.1f, 1),
            constrain(reproductionRate + random(-0.01f, 0.01f), 0.1f, 1),
            constrain(mutationChance + random(-0.01f, 0.01f), 0.05f, 0.3f),
            constrain(visionRange + random(-5, 5), 20, 200),
            constrain(aggressiveness + random(-0.1f, 0.1f), 0, 1),
            constrain(resilience + random(-0.1f, 0.1f), 0, 1)
        );
    }
}
...
class Creature {
    ...
    Creature(Gene genes) {
        ...
        energy = 200;
        age = 0;
        isAggressive = this.genes.aggressiveness > 0.5f;
        creatureType = (int) random(3);
        displayColor = color(this.genes.colorR, this.genes.colorG, this.genes.colorB, 200);
    }

    public Gene randomGenes() {
        return new Gene(
            random(1, 5), random(0.5f, 3), random(50, 255), random(50, 255), random(50, 255),
            random(50, 200), random(0.1f, 1), random(0.1f, 1), random(0.05f, 0.3f),
            random(20, 100), random(0, 1), random(0.2f, 1)
        );
    }
    ...
    public void update() {
        ...
        energy -= genes.metabolism * 0.1f * simulationSpeed;
        
        if (isRaining) energy -= 0.05f * simulationSpeed;
    ...
    public void interactWithCreatures() {
        for (Creature other : creatures) {
            if (other != this && dist(pos.x, pos.y, other.pos.x, other.pos.y) < genes.visionRange) {
                ...
                } else if (creatureType == 2 && random(1) > 0.5f) {
    ...
    public void interactWithViruses() {
...
class Virus {
    ...
    public void infect(Creature target) {
...
class NeuralNetwork {
    ...
    public float decideAction(float foodInput, float creatureInput) {
        return map(responseToFood * foodInput + responseToCreatures * creatureInput, -1, 1, -0.5f, 0.5f);
    }
}
...
public void setupSimulationControls() {
...
public void updateAndDisplayCreatures() {
...
public void mouseWheel(MouseEvent event) {
    ...
    zoomLevel = constrain(zoomLevel - e * 0.05f, 0.5f, 2.5f);
}
...
public void saveToArchive(Creature creature) {
...
public void loadRandomCreatureFromArchive() {
...
public void displayControlsScreen() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--stop-color=#cccccc", "Artificial_life_simulator" };
    if (passedArgs != null) {
